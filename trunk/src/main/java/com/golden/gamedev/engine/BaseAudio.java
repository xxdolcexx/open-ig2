...
public class BaseAudio implements Runnable {
	...
	private List<String> audiofiles = new ArrayList<String>(); // Liste de fichier audio pour playlist
		...
		while (true) {
			...
			for (int i = 0; i < this.renderer.length; i++) {
				...
				if(!this.audiofiles.isEmpty()) {
					if(this.renderer[i].getStatus() == BaseAudioRenderer.END_OF_SOUND)
						this.play();
				} else {
	...
	/**
	 * Ajout de musique dans la playlist
	 * @param audiofile Name of the audio file to play.
	 */
	public void addMusic(String audiofile) {
		audiofiles.add(audiofile);
	}
	
	public int play() {
		if(audiofiles.isEmpty())
			throw new RuntimeException("No music in the playlist");
		
		if(this.lastAudioFile == null)
			return play(audiofiles.get(0));
		else {
			for(int i=0 ; i < audiofiles.size() ; i++) {
				if(this.lastAudioFile.equalsIgnoreCase(audiofiles.get(i)))
					if(audiofiles.size() == i-1)
						return play(audiofiles.get(0));
					return play(audiofiles.get(i+1));
			}	
		}
		
		throw new RuntimeException("No music match the last audio files");
	}
